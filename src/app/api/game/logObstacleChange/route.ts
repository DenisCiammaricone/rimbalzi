import { isSessionCodeValid, logChangeCellObstacle, logLevelSwitch } from "@/actions/game";
...
export async function POST(req: Request) {
    ...
    try {
        ...
        if(body.session_code && await isSessionCodeValid(sessionCode)) {
            try {
                if (pupilCode) {
                    await logChangeCellObstacle(body.level, body.x, body.y, body.starting_obstacle, body.new_obstacle, sessionCode, pupilCode.value);
